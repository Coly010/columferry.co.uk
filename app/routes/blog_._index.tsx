...
export const meta: MetaFunction = () => {
    return [
        ...
        {
            name: "description",
            content: "Take a look through my blog posts on everything from Tech to Writing. Find articles about Angular, Nx, Module Federation and more here."
        },
    ];
};
...
export default function Blog__index() {
    ...
    return <div className="mt-4">
        <h1 className="text-left text-3xl py-4 mt-4 font-bold animate__animated animate__fadeInUp animate__fast underline decoration-emerald-300 decoration-4">Welcome
            to my blog</h1>
        <h2 className="text-xl text-slate-700 py-4 font-semibold animate__animated animate__fadeInUp animate__delay-1s">Here
            you will find a collection of articles I have written throughout my career, from tech articles to
            things I've learned since I began publishing fictional works.</h2>
        <div className="mt-4 grid animate__animated animate__fadeInUp animate__delay-2s">
            {data.map(post => <BlogCard key={post.slug} {...post} />)}
        </div>
    </div>
}

const BlogCard = ({slug, title, image, publishedDate, canonical, tags}: BlogListing) => {
    return <Link
        ...
        <div className="py-4 px-2">
            {publishedDate &&
                <h4 className="text-xs font-light pb-2">{canonical ? 'Originally posted' : 'Posted'} on <HighlightedText>{publishedDate}</HighlightedText>{canonical && (
                    <span> at <HighlightedText>{new URL(canonical).hostname}</HighlightedText></span>)}
                </h4>}
            <h2 className="font-semibold text-xl">{title}</h2>
            <div className="flex gap-1 mt-4">
                {tags && tags.map(tag => <BlogTag key={`${title}-${tag}`} tag={tag}/>)}
            </div>
        </div>
    </Link>
}
...
const BlogTag = ({tag}: { tag: string }) => {
    ...
    return <span className={`${BlogTagColors[tag] ?? 'bg-emerald-300'} rounded-xl px-2 py-[1px] text-xs`}>{tag}</span>
}
