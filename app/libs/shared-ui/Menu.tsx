...
import {faGithub, faAmazon, faXTwitter} from '@fortawesome/free-brands-svg-icons';
...
export const Menu = () => {
    return (
        <nav className="flex items-center justify-between p-4 relative">
            ...
            <div
                className="fixed bottom-0 right-1/2 z-50 h-10 pointer-events-none transform translate-x-1/2 -translate-y-1/2 lg:block lg:relative lg:transform-none lg:right-auto xl:absolute xl:w-full xl:-ml-4 xl:top-4 xl:flex xl:justify-center select-none">
                <div
                    className="pointer-events-auto rounded-xl w-80 h-10 p-0.5 bg-white border border-zinc-300 shadow-sm flex justify-between items-center gap-x-1">
            ...
            <div className="flex justify-between items-center gap-2">
                <MenuButton Icon={<FontAwesomeIcon icon={faGithub}/>}
                            to="https://github.com/Coly010"/>
                <MenuButton Icon={<FontAwesomeIcon icon={faXTwitter}/>}
                            to="https://x.com/FerryColum"/>
                <MenuButton Icon={<FontAwesomeIcon icon={faAmazon}/>}
...
const NavPill = ({children, to}: { children: React.ReactNode, to: string }) => {
    return (<NavLink
        to={to}
        className={({
                        isActive,
